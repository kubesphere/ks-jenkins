  ...
  while(true) {
    ...
    if (initLevel >= InitMilestone.PLUGINS_STARTED) {
      ...
      if (zhPlugin != null) {
        ...
        File initFile = new File(instance.getRootDir(), "/war/WEB-INF/init.groovy.d/remove-bundle-plugins.groovy");
        if(initFile.isFile()) {
            initFile.renameTo(new File(instance.getRootDir(), "/war/WEB-INF/init.groovy.d/remove-bundle-plugins.groovy.bak"));
        }
